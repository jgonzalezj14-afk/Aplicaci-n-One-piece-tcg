...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _fetchTopLeaders() async {
    try {
      final leaders = await _apiService.getCardsByIds(['OP13-079', 'OP13-002', 'OP12-020']);
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _goldColor, foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 15),
                      textStyle: const TextStyle(fontWeight: FontWeight.bold), elevation: 10, shadowColor: _goldColor.withOpacity(0.5),
                    ),
                  ),
  ...
  Widget _buildSectionTitle(IconData icon, String title) {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: _goldColor, size: 28), const SizedBox(width: 10), Text(title, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold, letterSpacing: 1))]);
  }
